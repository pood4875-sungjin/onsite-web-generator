...
```bash
for u in "app/studio/studio.html" "app/index.html" "app/llm.js" "app/packs.machine.js" "app/bg/gennx-1.jpg" "app/bg/dubai-hero.jpg"; do
  printf '%s ' "$u"; curl -sL -o /dev/null -w '%{http_code}\n' "https://midas-drs.pages.dev/$u"; done
```
- 코드 내용 확인: `curl -s "https://midas-drs.pages.dev/app/packs.X.js?v=$(date +%s)" | grep -c "핵심문자열"
