...
(function () {
  ...
  var HFD_ALLOWED = { cover: 1, greeting: 1, toc: 1, divider: 1, section: 1, cards: 1, timeline: 1, table: 1, checklist: 1, media: 1, photos: 1, quote: 1, closing: 1, stats: 1, kpi: 1, process: 1, compare: 1, roadmap: 1, milestone: 1, split: 1, bigstat: 1, word: 1, statement: 1, hero: 1 };
  ...
  async function composeDeck(brief, onText) {
    brief = brief || {};
    // 덱 콘텐츠 언어 — UI 언어를 따른다(영어 시연 등). BYOK 경로는 시스템 프롬프트에 직접 주입.
    var _ol = brief.lang || uiLang();
    var _LR = _ol === 'ko' ? '' : '\n[Output language — overrides any Korean instruction above] Write ALL slide text (titles, body, lists, captions, note) in ' + ({ en: 'English', ja: 'Japanese', zh: 'Simplified Chinese' }[_ol] || 'English') + '. Keep brand names and proper nouns as-is.';
    ...
    if (brief.pack === 'machine' && window.MACHINE_SCHEMA_DOC) {
      var msys =
        ...
        '슬라이드 타입은 내용 성격에 맞춰 아래 "언제 쓰나"로 고른다(같은 타입만 반복 금지):\n' + window.MACHINE_SCHEMA_DOC + '\n' +
        '각 타입의 필드: ' + window.MACHINE_FIELD_DOC + '\n' +
        '규칙: 1장 cover(title은 영문 2~3줄 권장, **볼드**·__딤__ 3톤). 2장 statement 또는 toc(items의 label=영문 챕터명 대문자). ' +
        '섹션 전환은 darkhero 또는 demo. 진행 현황은 progress(rows pct 실제 수치, 핵심 행 on:true). 본문 장은 note(풋라인, **강조** 1회)를 적극 넣는다. ' +
    ...
    if (brief.pack === 'sfmi' && window.SFMI_SCHEMA_DOC) {
      var fsys =
        '너는 시니어 발표 기획자다. 브리프로 한국어 프레젠테이션 슬라이드 덱을 설계한다.\n' +
        '반드시 유효한 JSON 하나만 출력한다. 코드펜스·주석·설명 문장 금지. 형식: {"slides":[...]}\n' +
        '슬라이드 타입은 내용 성격에 맞춰 아래 "언제 쓰나"로 고른다(같은 타입만 반복 금지):\n' + window.SFMI_SCHEMA_DOC + '\n' +
        '각 타입의 필드: ' + window.SFMI_FIELD_DOC + '\n' +
        '규칙: 1장 cover. 2장 statement 또는 toc. toc items=divider title과 1:1. ' +
        '챕터마다 divider(title=영문 1~3단어 대문자, lead=국문 헤드라인 — 챕터 컬러 자동). 본문 장은 밀도 있게 — note(풋라인 문장, **강조** 1회)를 적극 넣는다. ' +
  ...
  // 웹/랜딩 자연어 수정: {site, instruction} → {site|null, message}. PPT editDeck과 같은 계약.
  async function editSite(site, instruction) {
    var txt;
    if (usingProxy()) {
      var r = await fetch(proxyUrl() + '/edit-web', {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ site: site, instruction: instruction, lang: uiLang() }),
      });
      var j = null; try { j = await r.json(); } catch (e) {}
      if (!r.ok) throw new Error(_proxyErrMsg(j, r.status));
      txt = j.text;
  ...
  async function _translateOne(payload, to) {
    ...
    if (usingProxy()) {
      var r = await fetch(proxyUrl() + '/translate', {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ payload: payload, to: to || 'en' }),
      });
      var j = null; try { j = await r.json(); } catch (e) {}
      if (!r.ok) throw new Error(_proxyErrMsg(j, r.status));
      txt = j.text;
    } else {
      var tname = { ko: '한국어', en: 'English', ja: '日本語', zh: '简体中文' }[to || 'en'] || 'English';
      var tsys =
        '너는 전문 로컬라이저다. 받은 JSON의 문자열 값만 전부 ' + tname + '로 번역해 같은 구조로 돌려준다.\n' +
        '반드시 유효한 JSON 하나만 출력: {"payload":{...입력과 같은 구조...}}\n' +
        '키·구조·숫자·불리언 불변. **·__ 마크업과 \\n 유지. 브랜드 고유명사·type 등 시스템 값·영문 대문자 디자인 라벨은 그대로.';
