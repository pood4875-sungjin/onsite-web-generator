...
(function () {
  ...
  function allowedFor(pack) { return pack === 'sfmi' ? NAVER_ALLOWED : pack === 'pastel' ? NAVER_ALLOWED : pack === 'rams' ? NAVER_ALLOWED : pack === 'naver' ? NAVER_ALLOWED : pack === 'honors' ? HONORS_ALLOWED : pack === 'pitch' ? PITCH_ALLOWED : ALLOWED; }
  ...
  async function composeDeck(brief, onText) {
    ...
    if (brief.pack === 'rams' && window.RAMS_SCHEMA_DOC) {
      var rsys =
        ...
        '총 장수: short=5~8, std=10~15, deep=20~24, 없으면 6~12(목차·간지 포함). ' +
        '작법: 한 장에 한 메시지·제목만 이어 읽어도 논리 성립·첫 3장 내 왜-지금·마지막에 다음 행동·연속 3장 같은 골격 금지·수치엔 출처, 추정은 표기.';
      var rtxt = await messages({ system: rsys, user: '브리프:\n' + JSON.stringify({ title: brief.title || '', message: brief.message || '', audience: brief.audience || '', plan: brief.plan || '', length: brief.length || '', outline: brief.outline || [] }, null, 2), maxTokens: 8000 });
  ...
  async function editDeck(slides, instruction, pack) {
    ...
    var sys =
      ...
      '슬라이드 스키마: ' + (pack === 'sfmi' && window.SFMI_FIELD_DOC ? window.SFMI_FIELD_DOC : pack === 'pastel' && window.PASTEL_FIELD_DOC ? window.PASTEL_FIELD_DOC : pack === 'rams' && window.RAMS_FIELD_DOC ? window.RAMS_FIELD_DOC : pack === 'naver' && window.NAVER_FIELD_DOC ? window.NAVER_FIELD_DOC : pack === 'honors' && window.HONORS_FIELD_DOC ? window.HONORS_FIELD_DOC : pack === 'pitch' && window.PITCH_FIELD_DOC ? window.PITCH_FIELD_DOC : SCHEMA_DOC) + '\n' +
