...
const RAMS_SYSTEM =   // 자유 구성(2026-08-11 시연 잠금 해제)
  '너는 시니어 발표 기획자다. 브리프로 한국어 프레젠테이션 슬라이드 덱을 설계한다.\n' +
  '반드시 유효한 JSON 하나만 출력한다. 코드펜스·주석·설명 문장 금지.\n' +
  '형식: {"slides":[ ... ]}\n' +
  '슬라이드 타입은 내용 성격에 맞춰 아래 "언제 쓰나"로 고른다(같은 타입만 반복 금지):\n' + RAMS_USE_DOC + '\n' +
...
const HFD_USE_DOC = "cover(표지): 첫 장 — 2밴드+원 기하, 행사명·서브·날짜\ngreeting(인사말): 대표·주최 인사 문장 한 단락\ntoc(목차): 안내 순서 — 컬러 밴드 컬럼 2~4\ndivider(간지): 파트 시작 — 표지 기하+번호+타이틀\nsection(본문 표준): 핵심 안내 2~4개 — 번호+보더탑 소제목\ncards(N열 카드): 프로그램·혜택 카드 2~4 — 틴트/딥 셀\ntimeline(일정): 당일 시간표 — 시간+제목+설명 행\ntable(표): 장소·대상·문의 등 정보 표\nchecklist(체크리스트): 준비물·유의사항 — 도트 리스트, 5개 초과 시 2열\nmedia(안내 rows): 오시는 길·운영 정보(라벨+내용, 핵심 행 강조)+이미지 슬롯\nphotos(포토 앨범): 행사 사진 장 — wide 풀폭+캡션 밴드/grid 패널 안 3장/frame 화이트 프레임/side 좌 캡션+우 패널\nstats(수치): 좌 대형 % 카드+우 진행바\nkpi(KPI 카드): 값 카드 2~4\nprocess(프로세스): 단계 3~4 화살표\ncompare(비교): 작년/올해 카드 2\nroadmap(로드맵): Now/Next/Then 3열\nmilestone(마일스톤): 준비 계획 간트\nsplit(좌우 대비): 두 축 리스트\nbigstat(대형 수치): 숫자 임팩트\nduo(수치 2패널): 대형 수치·문구 두 개 나란히 — 값+라벨+구성 칩\nflow(전환 구조도): 전 → 후 변화 구조 — 좌 이전 패널, 화살표, 우 이후 패널 1~2\nhsteps(가로 단계): 일정·절차를 가로 노드 4~6개로 — 도트+라인+날짜\nprofile(프로필 카드): 팀·부스·조직 소개 2~3열 — 뱃지+딥 헤더+리스트+포커스 박스\nband(와이드 밴드): 딥 밴드(선언+포인트 2~3)+하단 카드 3~5 — 개요 한 장 요약\nhalfimg(하프 이미지): 좌 딥 텍스트 패널+우 절반 이미지(사용자 업로드) — 브랜드·인사 선언 장\nchart(차트+노트): 좌 막대 차트+우 코멘트 리스트 — 추이·실적(수치는 브리프에 있는 것만)\ncycle(순환 다이어그램): 상하 아크+중앙 허브 — 주고받는 관계·선순환 구조\nmatrix(매트릭스): 좌 대형 이니셜+우 그룹 패널 행 2~3 — 내용 많은 분류 정리\ntriple(3분할 보드): 헤더 밴드 3열 — 열마다 리드+리스트, 마지막 열 강조\nquad(4분할): 사분면 4블록+중앙 원 허브 — 영역별 실행 항목\norg(구조도): 상단 조직 박스(밴드+칩)+커넥터+하단 조직 박스 — 계층 구조\nlineup(라인업): 프로그램·연사·부스 라인업 행 3~5 — 태그+이름+설명+상태 뱃지, 첫 강조\nbranch(분기 구조): 상단 리드 → 하위 카드 2~4 분기 — 조직·역할 갈래\nhighlight(하이라이트): 대형 번호 재생 행 2~3 — 핵심 포인트 임팩트\nboard(보드): 좌 카드 2 스택+우 딥 사이드(리스트+필) — 두 축+요약\nlist(번호 리스트): 번호 원 카드 행 2~5 — 절차·핵심 포인트, 첫 행 강조\ndash(지표 대시보드): 미니 차트 카드 3~4+하단 스탯 스트립(수치는 브리프에 있는 것만)\nword(한 단어): 한 단어 임팩트 — 대형 타이포 중앙(감사·환영)\nstatement(스테이트먼트): 타이틀+서브 문장 중앙 대형 — 메시지 전달 장(variant frame=그린 프레임 보더+센터 문단)\nhero(이미지 히어로): 풀블리드 배경 이미지+중앙 텍스트(이미지는 사용자 업로드)\nquote(슬로건): 행사 슬로건·메시지 — 풀블리드 딥 밴드\nagenda(아젠다): 발표 순서 — 잉크 패널에 파트 뱃지|제목|발표자 행 2~4\nsidebar(사이드바 본문): 좌 컬러 사이드바 제목+우 리드·문단 — 서술형 본문(variant num=절반 바+대형 숫자)\nscreen(스크린 패널): 딥 지면+좌 연도 뱃지·타이틀·리스트+우 대형 이미지 패널(이미지는 사용자 업로드)\nnote(노트): 타이틀+풀폭 헤어라인+좌 소제목·우 문단 — 담백한 텍스트 장(variant band=상단 밴드)\ncircles(원형 수치): 겹침 원 2~3에 태그·대형 수치·라벨 — 목표·성과 강조(수치는 브리프에 있는 것만)\nclosing(엔딩): 마지막 장 — 표지 기하+마무리 인사·문의처";
const HFD_FIELD_DOC = "cover:{title(행사명, \\n 2줄),sub?(국문 부제),date?(일시),variant?:\"pill\"(2026 필 기하 — kicker?=좌상 라벨)|\"pill2\"(타이틀 좌상)} | greeting:{label?,text(인사 문장, **강조**, \\n 줄바꿈),by?(주최명)} | toc:{title?,items:[{no?,label(파트명),desc?(한 줄),pages?}](2~4개)} | divider:{no?:\"01\",title(파트 제목),lead?(한 문장),variant?:\"pill\"(2026 필 기하 — 타이틀 \\n 3~4줄)|\"pill2\"} | section:{title(\\n 2줄 가능),points:[{no?,head,text}](2~4개),sub?,note?(하단 밴드 문장)} | cards:{title,cards:[{tag?,head,text?,tone?:\"dark\"(딥 셀)}](2~4개),sub?,note?} | timeline:{title,items:[{when:\"13:00\",head,text?,on?:true(틴트 강조)}](3~6개),sub?,note?} | table:{title,columns:[str],rows:[{cells:[str]}],sub?,note?} | checklist:{title,items:[str],cols?:1~2,sub?,note?} | media:{title,specs:[{label(짧은 영문),text,on?:true(딥 강조 행)}](3~5개),image?:{label},caption?,sub?,note?} | photos:{variant?:\"wide|grid|frame|side|quad|frame3\"(기본 wide — quad=좌 사이드바+2×2 도트 캡션, frame3=딥 지면+화이트 프레임 3),year?(연도 칩 \"2026년\"),title(캡션 밴드 타이틀 \\n 2줄),caption?(우측 캡션 \\n 2~3줄),foot?,items:[{label,caption?(quad 셀 캡션)}](wide·frame 1개, grid·frame3 3개, quad 4개)} |stats:{title,donut?:{pct:0~100,label?,caption?},bars?:[{label,pct:0~100,value?,on?:true(틴트 강조 행),text?}](2~4개),sub?} | kpi:{title,items:[{value,label,desc?,tone?:\"on\"(딥 카드)}](2~4개),sub?} | process:{title,steps:[{tag:\"STEP 1\"류,head,text?}](3~4개),accent?:강조 인덱스(기본 중앙),sub?} | compare:{title,items:[{head:\"작년|올해\"류,items:[str]}](2개),sub?} | roadmap:{title,steps:[{when:\"Now|Next|Then\",head,items:[str],state?:\"now\"}](3개),sub?} | milestone:{title,phases?:[{tag,head,text?,on?:true}](2~3),bars:[{label,sub?,start:1~축개수,span:칸수}](3~5 시간순 계단),axis:[구간 라벨 4~6]} | split:{title?,left:{kicker,items:[str]},right:{kicker,items:[str]},sub?} | bigstat:{title,value,caption?(**강조**)} | duo:{title,items:[{value,label,text?,chips?:[str],tone?:\"on\"}](2개),sub?} |flow:{title,from:{head,items:[{k,v}|str]},to:[{head,items:[{k,v}|str],tone?:\"on\"}](1~2개),foot?,sub?} | hsteps:{title,steps:[{when,head,text?}](4~6개),sub?} | profile:{title,cards:[{badge?,kicker?,head,points:[str],focus?:{label,items:[str]},foot?}](2~3개),sub?} | band:{title,lead(\\n 2줄),points?:[{head,text}](2~3개),cards:[{head,text?,tone?:\"on\"}](3~5개),sub?} | halfimg:{head(\\n 2줄),lead?,text?}(이미지는 사용자 업로드 — 창작 금지) | chart:{title,bars:[{x,v(숫자),label?,on?:true}](3~8개, 브리프 수치만·창작 금지),notes?:[{head,text}](2~4개),badge?,unit?,sub?} | cycle:{center(\\n 2줄),top:{head,text?},bottom:{head,text?},left?:{head,text?}|str,right?:{head,text?}|str,topLabel?,bottomLabel?,title?} |matrix:{title,rows:[{tag(대형 이니셜/2자),label,sub?,groups:[{head,items:[str 2~4개]}](1~3개)}](2~3개),sub?} | triple:{title,cols:[{head,lead?(**강조**),items?:[str],foot?,tone?:\"on\"}](3개),sub?} | quad:{center:{label,head},cells:[{head,items:[str 2~3개]}](4개),title?} | org:{top:{head,cap?,label?,band?,items:[str 2~4]},bottom:{head,items:[str 2~4]},title?} | lineup:{title,items:[{tag?,head,text?,badge?,state?:\"dim\"|\"on\"}](3~5개, 첫 항목 자동 강조),sub?} | branch:{title,lead:{label?,text},branches:[{label?,head,text?}](2~4개),sub?} | highlight:{title,items:[{no?,head(\\n 2줄),text?}](2~3개),footnote?,sub?} | board:{title,cards:[{tag?,head(\\n 2줄),text?}](2개),side?:{title?,items:[str],pills?:[str]},sub?} | list:{title,rows:[{label,sub?}](2~5개),accent?,sub?} |dash:{title,cards:[{label,value,unit?,chart?:{kind:\"bars\"|\"donut\"|\"area\",v?:[숫자],pct?:숫자},text?}](3~4개, 브리프 수치만),strip?:[{label,value}](1~3개),sub?} | word:{text(2~6자 대형 단어),caption?(한 줄)} | statement:{title(중앙 대형 문장, \\n 2줄),sub?(한 줄),variant?:\"frame\"(그린 프레임 보더 — text?=센터 문단 \\n 여러 줄)} |hero:{title(중앙 대형, \\n 2줄),sub?(한 줄)}(배경 이미지는 사용자 업로드 — 창작 금지) | quote:{text(슬로건, **강조**, \\n 줄바꿈),by?} | agenda:{title?(기본 AGENDA),rows:[{part?(\"1부\"류 뱃지),head(발표 제목),who?(발표자 | 소속, \\n 여러 명)}](2~4개)} | sidebar:{title(사이드바 제목 \\n 2줄),lead?(볼드 리드 \\n 2줄),text(문단 \\n 줄바꿈),variant?:\"num\"(절반 사이드바+대형 숫자),no?(\"1\")} | screen:{title(좌 타이틀 \\n 2줄),year?(뱃지 \"2025년\"),points?:[str](2~5개)}(우측 패널 이미지는 사용자 업로드 — 창작 금지) | note:{title(한 줄 제목),head?(좌 소제목 \\n),text(우 문단 \\n 줄바꿈),variant?:\"band\"} | circles:{title,sub?,items:[{tag?(\"A. 목표\"류),value(대형 수치 — 브리프에 있는 것만·창작 금지),label?}](2~3개)} | closing:{title(마무리 인사, \\n 2줄),sub?,contact?(문의처)}\n규칙: 행사 안내 톤 — 따뜻하고 간결하게. 일시·장소·대상·프로그램은 브리프에 있는 것만 쓰고 창작 금지. title은 의미 단위 \\n 줄바꿈. 이모지 금지. 덱 루트에 theme(green|teal|cyan|indigo) 1개.";
const HFD_SYSTEM =
...
const MACHINE_SYSTEM =   // 자유 구성(2026-08-11 시연 잠금 해제)
...
const MACHINE_SYSTEM_LOCKED =   // [보존] 시연 잠금본 — 재잠금 시 이름을 MACHINE_SYSTEM으로
  '너는 MIDAS GEN NX 제품 소개 발표를 만드는 시니어 프로덕트 마케터다.\n' +
  '반드시 유효한 JSON 하나만 출력한다. 코드펜스·주석·설명 문장 금지. 형식: {"slides":[ ... ]}\n' +
...
const NAVER_FIELD_DOC =
  ...
  'closing:{title,sub?,contacts?:[{k,v}]}';
const NAVER_SYSTEM =   // 자유 구성(2026-08-11 시연 잠금 해제)
  '너는 시니어 발표 기획자다. 브리프로 한국어 프레젠테이션 슬라이드 덱을 설계한다.\n' +
  '반드시 유효한 JSON 하나만 출력한다. 코드펜스·주석·설명 문장 금지.\n' +
  '형식: {"slides":[ ... ]}\n' +
  '슬라이드 타입은 내용 성격에 맞춰 아래 "언제 쓰나"로 고른다(같은 타입만 반복 금지):\n' + NAVER_USE_DOC + '\n' +
...
const NAVER_SYSTEM_LOCKED =   // [보존] 시연 잠금본 — 재잠금 시 이름을 NAVER_SYSTEM으로
...
export default {
  async fetch(req, env) {
    ...
    if (route === '/compose') {
      ...
      /* 화면 비율(계약 v1) — 지면 모양이 달라지면 "한 장에 담기는 분량"이 달라진다.
         언어 주입과 같은 방식으로 팩 선택 이후 조건부 append. 16:9(기본)는 no-op.
         ⚠️ 분량 지시만 — 장수·슬라이드 타입·잠금 문구는 절대 건드리지 않는다(시연 잠금 유지). */
      const deckRatio = clip(body.ratio, 8) || '16:9';
      if (deckRatio === '3.8:1') system += '\n[지면 분량 — 화면 비율 3.8:1(초와이드, 2736×720)] 장수·슬라이드 타입 구성은 위 지시 그대로 두고, 각 장의 "내용 밀도"만 조정한다. 가로가 매우 넓고 세로가 낮으므로 한 장에 항목을 더 많이 담는다(카드·리스트·컬럼류는 5~6개 권장). 대신 문장은 짧게 — 각 항목의 설명은 한 줄(공백 포함 40자 내외)로 끊고, 긴 서술형 문단은 쓰지 않는다.';
      else if (deckRatio === '4:3') system += '\n[지면 분량 — 화면 비율 4:3(클래식, 1280×960)] 장수·슬라이드 타입 구성은 위 지시 그대로 두고, 각 장의 "내용 밀도"만 조정한다. 세로가 넉넉하므로 위에서 아래로 쌓이는 구성이 유리하다. 한 장의 항목은 3~4개로 줄이고, 각 항목의 설명 문장은 여유 있게(두 줄 정도) 쓴다. 가로로 넓게 늘어놓는 구성은 피한다.';
