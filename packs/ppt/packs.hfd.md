...
- **인덱스형 이미지 키(card-2-0·photos-3-1)는 부분 렌더에서 시프트된다** — 썸네일 한 장·선택 내보내기는 슬라이드가 slides.0이 되므로 studio `remapImagesFor(idxs)`로 키를 재배치해 넘겨야 함(2026-08-24 "이미지 첨부 안 됨" 실사고 — 원인 5곳 전부 studio: 폴백 덱 편집모드 미진입·키 시프트·레일 미갱신·교체 버튼이 slide.imgs 계약으로 오라우팅·드롭 미지원).
