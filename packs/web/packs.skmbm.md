...
`shared.variants = { 섹션: 변형명 }` — 미지정 시 기본형. 전체 사전 = docs/sections-skmbm.html — 피그마 "행사 랜딩 12섹션" 정의 체계(Design Generator 484-83314)로 그룹핑된 82종. 그룹 헤더에 피그마 정의(역할·반복·CTA·데이터 필드), 라이브 프레임(프레임 내 스크롤=실모션)·테마 5종 스위처·전체 페이지 모달.
